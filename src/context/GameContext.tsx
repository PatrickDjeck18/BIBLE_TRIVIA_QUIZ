...
type GameAction =
    ...
    | { type: 'COMPLETE_DAILY_CHALLENGE'; payload: { points: number; date: string } }
    | { type: 'RESET_PROGRESS' };
...
function checkAndAddAchievements(state: GameState): string[] {
    ...
    const completedLevels = Object.values(state.levelsProgress || {}).filter(l => l.completed);
    const completedCount = completedLevels.length;
    const perfectCount = completedLevels.filter(l => l.stars === 3).length;

    if (completedCount >= 1) {
        addIfMissing('first_steps');
    }
    if (completedCount >= 10) {
        addIfMissing('scholar');
    }
    if (perfectCount >= 5) {
        addIfMissing('perfect');
    }
    if (state.currentStreak >= 3) {
        addIfMissing('streak_3');
    }
    if (state.currentStreak >= 7) {
        addIfMissing('streak_7');
    }
    return achievements;
}
...
function gameReducer(state: GameState, action: GameAction): GameState {
    switch (action.type) {
        ...
        case 'COMPLETE_LEVEL': {
            ...
            const updatedUnlocked = shouldUnlockNext
            ...
            updatedState.achievements = checkAndAddAchievements(updatedState);
            return updatedState;
        }
        ...
        case 'COMPLETE_DAILY_CHALLENGE':
            return {
                ...state,
                totalPoints: state.totalPoints + action.payload.points,
                lastChallengeDate: action.payload.date,
...
interface GameContextType {
    ...
    completeDailyChallenge: (points: number) => void;
}
...
export function GameProvider({ children }: { children: ReactNode }) {
        ...
        try {
            ...
            if (savedState) {
                ...
                const mergedState: GameState = {
                    ...initialState,
                    ...parsed,
                    playerName: parsed.playerName !== undefined ? parsed.playerName : null,
                    achievements: parsed.achievements !== undefined ? parsed.achievements : [],
                    lastChallengeDate: parsed.lastChallengeDate !== undefined ? parsed.lastChallengeDate : null,
                    dailyChallengeCompleted: parsed.lastChallengeDate === today ? (parsed.dailyChallengeCompleted ?? false) : false,
                };
    ...
    const completeDailyChallenge = (points: number) => {
        const today = new Date().toISOString().split('T')[0];
        dispatch({ type: 'COMPLETE_DAILY_CHALLENGE', payload: { points, date: today } });
    };
